...
static inline uint8_t *getPKCS7PaddingInput(const char *in) {
    ...
    paddingInput[size] = '\0';
    LOGD("%s\nsize:%d\ninLength:%d", paddingInput, size, inLength);
    return paddingInput;
}
...
char *AES_CBC_PKCS7_Encrypt(const char *in, const uint8_t *key, const uint8_t *iv) {
    ...
    uint8_t *paddingInput = getPKCS7PaddingInput(in);
    int paddingInputLengt = strlen(paddingInput);
    LOGD("paddingInputLengt :%d",paddingInputLengt);
    char *out = (char *) malloc(paddingInputLengt);
...
/**
 * zreopadding，根据密钥长度自动选择128、192、256算法
 * 无论缺多少就补多少个0。再提醒一下就是如果刚满16个，那就要在补充称16个字节
 */
char *AES_CBC_ZREO_Encrypt(const char *in, const uint8_t *key, const uint8_t *iv) {
    ...
    paddingInput[size] = '\0';
    LOGD("paddingInputLengt :%d",size);
    char *out = (char *) malloc(size);
    AES_CBC_encrypt(out, paddingInput, size, key, iv);
    char *base64En = b64_encode(out, size);
    free(paddingInput);
    free(out);
    return base64En;
}
